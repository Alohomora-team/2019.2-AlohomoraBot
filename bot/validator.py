...
class ValidateForm:
    ...
    def cpf(cpf, update):
        ...
        # Validating CPF
        if ((int(cpf[9]) != 0 and authCPF_J != 0 and authCPF_J != 1)
            and (int(cpf[9]) != (11 - authCPF_J))):
            update.message.reply_text('CPF inválido, tente novamente:')
            logger.error("Invalid CPF - asking again")
            return False

        if((int(cpf[10]) != 0 and authCPF_K != 0 and authCPF_K != 1)
            and (int(cpf[10]) != (11 - authCPF_K))):
            logger.error("Invalid CPF - asking again")
            update.message.reply_text('CPF inválido, tente novamente:')
            return False
    ...
    def apartment(apartment, update):
        """
        Validate apartment information
        """
    ...
    def voice(voice_register, update):
        """
        Validate voice limit information
        """
    ...
    def boolean_value(value, update):
        """
        Parse portugues string to boolean value
        """
